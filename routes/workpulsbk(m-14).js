...
router.put('/syncworkpuls', function(req,res){
	...
	var options = [{
	 	...
	 	headers: {
		  ...
		  url: 'https://api.workpuls.com/v2/timeandattendance?start_date='+today,
		  headers: {
		      'auth_id' : '2fea5138-7029-4553-b3b3-bec9b5ffd254',
		      'auth_secret' : '8tthNgJZ'
		  }
	...
	async.map(options, function(obj, callback) {
	  ...
	  if (err) {
	    res.send('error')
	  		...
	  		res.json( productionData(results));				
...
router.put('/syncemployee', function(req,res){
	res.send("success ---------------------------");
	console.log("sdkjfherkthkdjhfgdg ------------------------------------------------------------------------------------------------")
	console.log( req.body.employee_id);
	console.log( req.body.clock_in);
