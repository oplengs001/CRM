...
(function ($, _, undefined) {
    ...
    var defaultSettings = {
        ...
        templates     : {
            ...
            mentionItemSyntax          : _.template('@[<%= value %>](<%= type %>:<%= _id %>)'),
        ...
        function populateDropdown(query, results) {
            ...
            if(!settings.allowRepeat) {
                ...
                results = _.reject(results, function (item) {
                    return _.include(mentionValues, item.fullname);
                });
            }
            ...
            _.each(results, function (item, index) {
                ...
                autocompleteItemCollection[itemUid] = _.extend({}, item, {value: item.fullname}); //Inserts the new item to autocompleteItemCollection

                var elmListItem = $(settings.templates.autocompleteListItem({
                    'id'      : utils.htmlEncode(item._id),
                    'display' : utils.htmlEncode(item.fullname),
                    'type'    : utils.htmlEncode(item.type),
                    'content' : utils.highlightTerm(utils.htmlEncode((item.display ? item.display : item.fullname)), query)
                ...
                if (settings.showAvatars) {
                    ...
                    if (item.avatar) {
                        elmIcon = $(settings.templates.autocompleteListItemAvatar({ avatar : item.path }));
